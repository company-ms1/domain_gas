...
<meta property="og:type" content="{'article' if filename == ARTICLE else 'website'}">
...
def cta(d):
    return f'''<aside class="cta"><div><h2>{e(d['ctaTitle'])}</h2><p>{e(d['ctaText'])}</p></div><a class="button" href="../../index.html">{e(d['service'])} ↗</a></aside>'''
...
<article class="prose">{sections}<details><summary>{e(d['faqTitle'])}</summary><p>{e(d['faqAnswer'])}</p></details>{cta(d)}<a class="back" href="index.html">← {e(d['back'])}</a></article></div>'''
...
def main():
    ...
    for lang in LANGUAGES:
        ...
        (folder / 'index.html').write_text(shell(lang, d, 'index.html', d['headline'], d['intro'], home(d)))
        (folder / ARTICLE).write_text(shell(lang, d, ARTICLE, d['title'], d['description'], article(d)))
    print('Built 8 pages in en/ ru/ es/ id/blog/.')
